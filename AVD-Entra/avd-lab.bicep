...
resource storageAccount 'Microsoft.Storage/storageAccounts@2023-01-01' = {
  ...
  properties: {
    ...
    allowSharedKeyAccess: false          // Enforce Entra identity — shared key access disabled
    ...
    azureFilesIdentityBasedAuthentication: {
      ...
      defaultSharePermission: 'StorageFileDataSmbShareContributor'  // Required for cloud SMB: enables permissions for all authenticated users
    }
  }
}
...
//     Uses Entra Kerberos authentication — no storage account key required.
//     The session host retrieves a Kerberos TGT at logon (via CloudKerberosTicketRetrievalEnabled,
//     deployed via Intune) and FSLogix uses that ticket to mount the profile share.
resource fslogixConfigRunCommand 'Microsoft.Compute/virtualMachines/runCommands@2023-09-01' = [for i in range(0, vmCount): {
  ...
  properties: {
    source: {
      script: '''
        param([string]$VHDLocation, [int]$SizeInMBs)
        ...
        Set-ItemProperty -Path $RegPath -Name AccessNetworkAsComputerObject        -Value 0            -Type DWord
        ...
        # ---- Entra Kerberos ticket retrieval ----
        # Allows the Kerberos TGT to be retrieved at logon so FSLogix can mount
        # the profile share using the user's Entra identity. Also managed by
        # Intune policy, but set here to ensure it is present immediately on
        # first login without waiting for Intune to deliver policies.
        $KerberosPath = 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa\Kerberos\Parameters'
        New-Item -Path $KerberosPath -Force | Out-Null
        Set-ItemProperty -Path $KerberosPath -Name 'CloudKerberosTicketRetrievalEnabled' -Value 1 -Type DWord
        Write-Output 'CloudKerberosTicketRetrievalEnabled set.'
